		...
		switch (state.mode) {
			...
			default:
				...
				if (col >= 0 && col <= 5) returnType = COBOLLINENUM;
					...
					state.mode = "string";
					returnType = STRING;
				} else if (ch == "'" && !tests.digit_or_colon.test(stream.peek())) returnType = ATOM;
				else if (ch == ".") returnType = PERIOD;
				else if (isNumber(ch, stream)) returnType = NUMBER;
				else {
					if (stream.current().match(tests.symbol)) while (col < 71) if (stream.eat(tests.symbol) === void 0) break;
					else col++;
					if (keywords && keywords.propertyIsEnumerable(stream.current().toUpperCase())) returnType = KEYWORD;
					else if (builtins && builtins.propertyIsEnumerable(stream.current().toUpperCase())) returnType = BUILTIN;
					else if (atoms && atoms.propertyIsEnumerable(stream.current().toUpperCase())) returnType = ATOM;
...
//# sourceMappingURL=cobol-OPxruUvo.js.map