...
function tokenBase(stream, state) {
	...
	if (/[\[\]{}\(\),\.]/.test(ch)) {
		...
		stream.eat(/./);
		return "number";
	} else if (ch == "/" && stream.eat("*")) return chain(stream, state, tokenComment);
	else if (ch == ";" && stream.match(/ *\( *\(/)) return chain(stream, state, tokenUnparsed);
		...
		if (specials && specials.propertyIsEnumerable(stream.current().toLowerCase())) return "keyword";
		else {
...
//# sourceMappingURL=mirc-Kt70675T.js.map