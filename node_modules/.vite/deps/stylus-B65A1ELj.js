...
states.block = function(type, stream, state) {
	...
	if (type == "variable-name") if (stream.string.match(/^\s?\$[\w-\.\[\]\'\"]+$/) || wordIsBlock(firstWordOfLine(stream))) return pushContext(state, stream, "variableName");
...
states.atBlock_parens = function(type, stream, state) {
	...
	if (type == ")") if (endOfLine(stream)) return pushContext(state, stream, "block");
...
states.keyframes = function(type, stream, state) {
	...
	if (type == "}") if (startOfLine(stream)) return popContext(state, stream, true);
...
//# sourceMappingURL=stylus-B65A1ELj.js.map