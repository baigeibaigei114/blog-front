...
function tokenBase(stream, state) {
	...
	if (state.lastToken != "\\" && stream.match("{%", false)) return chain(tokenMacro("%", "%"), stream, state);
	if (state.lastToken != "\\" && stream.match("{{", false)) return chain(tokenMacro("{", "}"), stream, state);
	if (stream.peek() == "#") {
		stream.skipToEnd();
		return "comment";
	...
	if (stream.eat("@")) {
		if (stream.peek() == "[") return chain(tokenNest("[", "]", "meta"), stream, state);
		stream.eat("@");
	...
	if (stream.eat(":")) {
		if (stream.eat("\"")) return chain(tokenQuote("\"", "atom", false), stream, state);
		else if (stream.match(idents) || stream.match(types) || stream.match(operators) || stream.match(conditionalOperators) || stream.match(indexingOperators)) return "atom";
		stream.eat(":");
		return "operator";
	}
	if (stream.eat("\"")) return chain(tokenQuote("\"", "string", true), stream, state);
	if (stream.peek() == "%") {
		var style = "string";
		var embed = true;
		var delim;
	...
	if (matched = stream.match(/^<<-('?)([A-Z]\w*)\1/)) return chain(tokenHereDoc(matched[2], !matched[1]), stream, state);
	if (stream.eat("'")) {
...
function tokenFollowIdent(stream, state) {
	...
	else stream.match(operators) || stream.match(conditionalOperators) || stream.match(indexingOperators);
	state.tokenize.pop();
	return "def";
}
...
//# sourceMappingURL=crystal-CfWigP6M.js.map