import { C as indentNodeProp, M as styleTags, N as tags, m as continuedIndent, n as ExternalTokenizer, r as LRParser, s as LRLanguage, t as ContextTracker, u as LanguageSupport, v as foldInside, y as foldNodeProp } from "./dist-CrgB1_Ed.js";
import { r as defineCSSCompletionSource } from "./dist-Cz-5Y8sD.js";
//#region node_modules/@lezer/sass/dist/index.js
...
var colon = 58, parenL = 40, underscore = 95, bracketL = 91, dash = 45, period = 46, hash = 35, percent = 37, braceL = 123, braceR = 125, slash = 47, asterisk = 42, newlineChar = 10, equals = 61, plus = 43, and = 38;
function isAlpha(ch) {
...
var comments = new ExternalTokenizer((input, stack) => {
	...
	if (stack.dialectEnabled(Dialect_indented)) {
		...
		if (indentedComment > -1) {
			...
			input.advance();
			while (input.next >= 0) if (input.next == newlineChar) {
				input.advance();
				let indented = 0;
				while (input.next != newlineChar && space.includes(input.next)) {
					indented++;
...
var indentation = new ExternalTokenizer((input, stack) => {
	...
	if (input.peek(-1) == newlineChar) {
		let depth = 0;
		while (input.next != newlineChar && space.includes(input.next)) {
			input.advance();
			depth++;
		}
		if (depth != cDepth && input.next != newlineChar && !startOfComment(input)) if (depth < cDepth) input.acceptToken(dedent, -depth);
...
//#endregion
//#region node_modules/@codemirror/lang-sass/dist/index.js
/**
...
var sassLanguage = /* @__PURE__ */ LRLanguage.define({
	name: "sass",
	parser: /* @__PURE__ */ parser.configure({ props: [/* @__PURE__ */ foldNodeProp.add({
		Block: foldInside,
		Comment(node, state) {
			return {
				from: node.from + 2,
...
var indentedSassLanguage = /* @__PURE__ */ sassLanguage.configure({
	dialect: "indented",
	props: [/* @__PURE__ */ indentNodeProp.add({ "Block RuleSet": (cx) => cx.baseIndent + cx.unit }), /* @__PURE__ */ foldNodeProp.add({ Block: (node) => ({
		from: node.from,
		to: node.to
	}) })]
});
...
//# sourceMappingURL=dist-Chu1P0Ho.js.map