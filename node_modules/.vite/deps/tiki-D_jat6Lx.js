//#region node_modules/@codemirror/legacy-modes/mode/tiki.js
...
function inText(stream, state) {
	...
	switch (ch) {
		...
		case "(":
			...
			break;
		case "[": return chain(inBlock("url", "]", inText));
		case "|":
			if (stream.eat("|")) return chain(inBlock("comment", "||"));
			break;
		case "-":
			if (stream.eat("=")) return chain(inBlock("header string", "=-", inText));
			else if (stream.eat("-")) return chain(inBlock("error tw-deleted", "--", inText));
			break;
		case "=":
...
var tiki = {
	...
	token: function(stream, state) {
		...
		if ((style || type) && style != "comment") {
			curState = state;
			while (true) if ((state.cc.pop() || element)(type || style)) break;
		}
...
//# sourceMappingURL=tiki-D_jat6Lx.js.map