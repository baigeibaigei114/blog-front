//#region node_modules/@codemirror/legacy-modes/mode/sql.js
function sql(parserConfig) {
	...
	function tokenBase(stream, state) {
		...
		if (support.hexNumber && (ch == "0" && stream.match(/^[xX][0-9a-fA-F]+/) || (ch == "x" || ch == "X") && stream.match(/^'[0-9a-fA-F]*'/))) return "number";
		else if (support.binaryNumber && ((ch == "b" || ch == "B") && stream.match(/^'[01]+'/) || ch == "0" && stream.match(/^b[01]*/))) return "number";
		else if (ch.charCodeAt(0) > 47 && ch.charCodeAt(0) < 58) {
			stream.match(/^[0-9]*(\.[0-9]+)?([eE][-+]?[0-9]+)?/);
			support.decimallessFloat && stream.match(/^\.(?!\.)/);
			return "number";
		} else if (ch == "?" && (stream.eatSpace() || stream.eol() || stream.eat(";"))) return "macroName";
		else if (ch == "'" || ch == "\"" && support.doubleQuote) {
			state.tokenize = tokenLiteral(ch);
			return state.tokenize(stream, state);
		} else if ((support.nCharCast && (ch == "n" || ch == "N") || support.charsetCast && ch == "_" && stream.match(/[a-z][a-z0-9]*/i)) && (stream.peek() == "'" || stream.peek() == "\"")) return "keyword";
			...
			return "operator";
		} else if (brackets.test(ch)) return "bracket";
		else if (punctuation.test(ch)) {
			stream.eatWhile(punctuation);
			return "punctuation";
		} else if (ch == "{" && (stream.match(/^( )*(d|D|t|T|ts|TS)( )*'[^']*'( )*}/) || stream.match(/^( )*(d|D|t|T|ts|TS)( )*"[^"]*"( )*}/))) return "number";
		else {
...
function hookIdentifier(stream) {
	var ch;
	while ((ch = stream.next()) != null) if (ch == "`" && !stream.eat("`")) return "string.special";
...
//# sourceMappingURL=sql-QgqB3Xr3.js.map